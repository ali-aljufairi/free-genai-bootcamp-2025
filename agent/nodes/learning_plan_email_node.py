...
def send_learning_plan_email_node(state: State):
    ...
    try:
        ...
        if not recipient_email:
            logger.warning(
                f"No email found in user_data for user_id {user_id}, skipping email"
            )
            return {}
        ...
        template_dir = os.path.join(
            os.path.dirname(__file__), "..", "utils", "email_templates"
        )
        env = Environment(
        ...
        goal_text = f" Your goal: {learning_goal}." if learning_goal else ""
        template_data = {
            "subject": f"Your Personalized Japanese Learning Plan - Ready to Start!",
            "heading": "Your Personalized Learning Plan",
            "introduction": f"Based on your progress and goals, we've created a customized study plan to help you improve your Japanese language skills.{goal_text}",
            "current_level_assessment": learning_plan.get(
