...
-- Note: study_activities, study_sessions, review_items, progress, shadow_attempts, kanji_traces
...
-- Add description and is_active columns if they don't exist
...
required_correct_ratio NUMERIC(3, 2) NOT NULL DEFAULT 0.80 CHECK (
    required_correct_ratio BETWEEN 0.1 AND 1.0
),
minimum_attempts INT NOT NULL DEFAULT 3 CHECK (minimum_attempts >= 1),
consecutive_correct INT NOT NULL DEFAULT 2 CHECK (consecutive_correct >= 1),
...
CREATE INDEX idx_learning_activities_scope ON learning_activities (
    course_id,
    unit_id,
    jlpt_level,
    group_id
...
CREATE INDEX idx_learning_activities_completed ON learning_activities (completed_at)
WHERE
...
CREATE INDEX idx_progression_settings_scope ON progression_settings (
    course_id,
    unit_id,
    jlpt_level,
    group_id
...
-- Note: study_activities, shadow_attempts, kanji_traces are dropped in 00006_learning_progress.sql
-- +goose StatementEnd