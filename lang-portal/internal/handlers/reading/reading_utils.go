...
func (h *ReadingHandler) getUserID(c *fiber.Ctx) (int64, error) {
...
// shuffleReadingOptions randomizes options and returns new slice and new correct index
func shuffleReadingOptions(answers []string, correctIndex int) ([]string, int) {
...
// validateReadingConfig validates reading quiz configuration
func (h *ReadingHandler) validateReadingConfig(config *ReadingQuizConfig) error {
	// Validate level
	...
	if config.QuestionType != ReadingQuestionTypeAll &&
		config.QuestionType != ReadingQuestionTypeInformationSearch &&
		config.QuestionType != ReadingQuestionTypeLongPassage &&
		config.QuestionType != ReadingQuestionTypeMediumPassage &&
		config.QuestionType != ReadingQuestionTypeComprehensive &&
		config.QuestionType != ReadingQuestionTypeTopic &&
		config.QuestionType != ReadingQuestionTypeShortPassage {
